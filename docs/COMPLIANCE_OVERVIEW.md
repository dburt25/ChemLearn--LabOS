...
- **UI Provenance Previews:** Wave-2 Control Panel updates expose Job/Dataset import previews and provenance summaries to keep users aware of lineage before promoting data.
...
- Automated UI validation for provenance previews and workspace tagging is still needed; current checks are manual only.
