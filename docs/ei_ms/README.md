...
print(result.fragments[1].classification)
print(result.summary["base_peak_mass"])
```
...
### Output
`run_basic_analysis` returns an `AnalysisResult` dataclass with:
...
- `notes`: Optional notes for downstream consumers.
- `message`: Reminder that the logic is heuristic only.

For callers that still expect a dictionary response, the compatibility helper
`run_ei_ms_analysis` forwards to `run_basic_analysis` and returns `AnalysisResult.to_dict()`.

## Module Registration

The module self-registers under the key `ei_ms.basic_analysis` with the operation `analyze` when imported. Add `labos.modules.ei_ms.basic_analysis` to `LABOS_MODULES` for autodiscovery or import the module directly to access `run_basic_analysis` (or the compatibility wrapper `run_ei_ms_analysis`).
